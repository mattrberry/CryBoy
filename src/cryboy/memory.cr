class Memory
  ...
  def initialize(@cartridge : Cartridge, @joypad : Joypad, @timer : Timer, @apu : APU, bootrom : String? = nil)
    if !bootrom.nil?
  ...
  def [](index : Int) : UInt8
    ...
    when SPRITE_TABLE           then @memory[index]
    ...
    when INTERRUPT_REG then @memory[index]
